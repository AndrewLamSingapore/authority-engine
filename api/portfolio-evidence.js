  ...
  try {
    if (req.method === 'POST') {
      ...
      const candidate = await client.createIfNotExists({
        _id: id, _type: 'portfolioEvidenceCandidate', eventId: assessment.event.event_id,
        eventType: assessment.event.event_type, source: assessment.event.source,
        evidenceLevel: assessment.event.evidence_level, status: assessment.status,
        reviewable: assessment.reviewable, redactions: assessment.redactions,
        policyReasons: assessment.reasons, eventJson: JSON.stringify(assessment.event),
    ...
    if (req.method === 'PATCH') {
      ...
      try {
        relay = await publishAuthorityEvent(authorityEvent('authority.evidence.reviewed', changed, { decision: req.body?.decision, source_event_id: changed.eventId || candidate.eventId }));
      } catch (error) {
        console.error(JSON.stringify({ level: 'error', message: 'authority_relay_failed', operation: 'review', error: String(error?.message || error) }));
        relay = { queued: false, reason: 'relay_failed' };
      }
      return res.status(200).json({ candidate_id: changed._id, status: changed.status, relay });
    }
    res.setHeader('Allow', 'GET, POST, PATCH');
    return res.status(405).json({ error: 'method_not_allowed' });
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', message: 'portfolio_evidence_failed', error: String(error?.message || error) }));
