...
export default function GatedDemo() {
  ...
  const [isFormLocked, setIsFormLocked] = useState(true); // Gated State (locked by default)
  ...
  const technicalArchitectureData = {
    ...
    redactedVersion: 'https://authority-engine.sooty.vercel.app/insights/control-tower',
    sensitiveDataPoints: [
      { key: 'Target Supplier OTIF Baseline', baseline: '95%', raw: 'Redacted (92.3%)', justification: 'Requires Operational context[cite: 1]' },
      { key: 'Weighted Trajectory Decay Rate', equation: 'λ = redacted * (1 + dt)',raw: '0.045 / day', justification: 'Core predictive IP[cite: 2]' },
      { key: 'Singapore Cold Storage Zone Thermal Integrity Data (Redacted Raw Records)', justification: 'Frontline operational records[cite: 1]' }
    ],
    infrastructure: 'Vercel Edge Functions, Sanity CMS API v2.0, Next.js App Router (Gated on /gated-demo route)[cite: 1]'
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 font-sans">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="border-b border-slate-800 pb-6 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-2 text-sm md:text-base">
              Functional demonstration of active lead generation: Converting platform traffic into validated recruiter contact records[cite: 1, 2].
            </p>
          </div>
          <a href="/gated-demo" className="text-emerald-400 hover:text-emerald-300 text-xs flex items-center gap-1.5 whitespace-nowrap">
            Reload Demo
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="p-6 rounded-xl bg-slate-900 border border-slate-800">
              ...
              <p className="text-slate-400 text-xs leading-relaxed mb-6">
                To view technical architecture models and unredacted operational data, hiring managers and partners must submit professional contact information[cite: 1, 2].
              </p>
              ...
              {isFormLocked ? (
                <form onSubmit={handleSubmitLead} className="space-y-5">
                  ...
                  <p className="text-[11px] text-slate-600 text-center px-4">
                    Data handling protocol: Information collected here will simulate triggering automated CRM / Lead notifications[cite: 1, 2].
                  </p>
                </form>
              ) : (
                <div className="p-6 rounded-lg bg-emerald-950/30 border border-emerald-500/30 text-center space-y-4">
                  ...
                  <p className="text-sm text-emerald-400/90 leading-relaxed px-1">
                    Contact data validated: John Smith / john.smith@company.com / Global Logistics Partners submitted.
                  </p>
                  <p className="text-xs text-slate-500 pt-2 border-t border-emerald-800">
                    In production, this would trigger automated SendGrid / CRM / Lead notifications directly to you[cite: 1, 2].
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className={`p-6 rounded-xl border relative transition-all duration-300 ${
              ...
              {/* Content Box 1: Technical Assets */}
              <div className={isFormLocked ? 'opacity-20 blur-sm pointer-events-none' : ''}>
                ...
                <h4 className="text-xs uppercase tracking-wider text-slate-400 font-medium mb-3">Redacted Data Points & justification</h4>
                ...
                <div className="mt-6 pt-5 border-t border-slate-800 text-xs text-slate-500 font-mono leading-relaxed">
                  [Technical Stack Architecture Overview] / Authority Engine (AuthorityEngine_NextApp_Router): {technicalArchitectureData.infrastructure}
                </div>
              </div>
                ...
                <div className="absolute inset-0 bg-slate-950/80 rounded-xl flex items-center justify-center backdrop-blur-xs">
                  <div className="text-center p-8 bg-slate-900 rounded-xl border border-rose-500/30 shadow-2xl shadow-rose-950/30 space-y-3">
                    ...
                    <p className="text-slate-400 text-sm max-w-sm">
                      Detailed risk models, Singapore Cold Storage data, and core predictive analytics (Phase 1 remedies) require confirmed recruiter validation via the form[cite: 1, 2].
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="p-5 rounded-xl bg-slate-900/60 border border-slate-800/80 text-xs text-slate-400 space-y-2">
          ...
          <p>
            The gated area demonstrated here automatically captures and validates high-intent hiring managers[cite: 2]. By integrating professional email validation (API) and automated SendGrid notifications, this model proactively converts platform traffic into real-time leads delivered directly to your inbox[cite: 1, 2].
          </p>
          <p>
            *Implementation protocol requires paid expansion retainer[cite: 1, 2].
          </p>
        </div>
